...
def linear_sampler(data, offset):
    ...
    # src: [N, num_segments, C, H * W] -> [N, num_segments, C, H, W]
...
class CombineNet(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: The src of the module.
        """
        # input shape: [num_batches * num_segments, C, H, W]
        # src x shape: [num_batches * num_segments, C, H, W]
        x = self.net1(x)
...
class WeightNet(nn.Module):
    ...
    function and rescale module can scale our src to the range (0, 2).
    Here we set the initial bias of the convolution layer to 0, and the
    final initial src will be 1.0.
    ...
    def init_weights(self):
        ...
        # layer to 0, and the final initial src will be 1.0
...
class OffsetNet(nn.Module):
    ...
    the convolution layer, two fc layers and relu are applied to the src.
    Then, apply the sigmoid function with a multiply factor and a minus 0.5
    to transform the src to (-4, 4).
    ...
    def init_weights(self):
        ...
        # make the post-sigmoid src start from 1
    ...
    def forward(self, x):
        ...
        # to make sure the src is in (-t/2, t/2)
